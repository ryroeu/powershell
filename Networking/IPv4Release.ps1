<#
.SYNOPSIS
    Releases IPv4 DHCP leases on Windows adapters.
#>
...
foreach ($target in $targets) {
    if ($PSCmdlet.ShouldProcess($target, 'Release IPv4 DHCP lease')) {
        $arguments = if ($target -eq '*') { @('/release') } else { @('/release', $target) }
